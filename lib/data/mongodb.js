...
db.prototype = {
    ...
    setTheme: function(room, theme) {
        this.rooms.update({ name: room }, { $set: { theme: theme } });
    },

    getTheme: function(room, callback) {
        this.rooms.findOne({ name: room }, { theme: true }, function(err, room) {
            if (room) {
                callback(room.theme);
            } else {
                callback();
    ...
    createColumn: function(room, name, callback) {
        this.rooms.update({ name: room }, { $push: { columns: name } }, { upsert: true }, callback);
    },
    ...
    deleteColumn: function(room) {
        this.rooms.update({ name: room }, { $pop: { columns: 1 } });
    },

    setColumns: function(room, columns) {
        this.rooms.update({ name: room }, { $set: { columns: columns } }, { upsert: true });
    },
    ...
    createCard: function(room, id, card) {
        var doc = {};
        doc['cards.' + id] = card;
        this.rooms.update({ name: room }, { $set: doc }, { upsert: true });
    },
    ...
    cardEdit: function(room, id, text) {
        ...
        this.rooms.update({ name: room }, { $set: doc });
    },
    ...
    deleteCard: function(room, id) {
        var doc = {};
        doc['cards.' + id] = true;
        this.rooms.update({ name: room }, { $unset: doc });
    },
    ...
    setBoardSize: function(room, size) {
        this.rooms.update({ name: room }, { $set: { size: size } });
    }
};
exports.db = db;
